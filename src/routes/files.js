...
// // Upload file
// router.post("/v1/file", upload.single("file"), async (req, res) => {
//   try {
//     const file = req.file;
//     if (!file) {
//       return res.status(400).json({ error: 'File key must be "file"' });
//     }
...
//         Bucket: priyesh-aws-bucket-testing-local,
//         Key: filePath,
//         Body: file.buffer,
...
//       url: `${priyesh-aws-bucket-testing-local}/${filePath}`, // Full URL to the file
...
// router.all("/v1/file/:fileId", (req, res, next) => {
//   if (
//     req.method !== "GET" &&
//     req.method !== "DELETE" &&
//     req.method !== "HEAD"
...
//       url: `${priyesh-aws-bucket-testing-local}/${fileMetadata.filePath}`, // Full URL to the file
...
// router.head("/v1/file/:fileId", async (req, res) => {
//   try {
//     const fileId = req.params.fileId;
//     const fileMetadata = await File.findOne({ where: { fileId } });
...
//     res.status(200).send(); // Return 200 with no body for HEAD requests
//   } catch (err) {
//     console.error("Error retrieving file metadata:", err);
//     res.status(404).send();
//   }
